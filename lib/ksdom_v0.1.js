...
document.querySelectorAll('body main *').forEach(element => {
    ...
    if (textContent.includes("\{\{") && textContent.includes("}}")) {
        ...
        domListeners.push({ element: rootNode, content: textContent, variables: variablesValues });
    }
});
...
function getKeysArrayFromHTMLString(str) {
    let results = [];
    ...
    updateListeners(variableName, newValue) {
        ...
        domListeners.forEach(listener => {
            ...
            Object.keys(listener.variables).forEach(variableKey => {
                ...
                let keysArray = getKeysArrayFromHTMLString(variableKey.trim());
